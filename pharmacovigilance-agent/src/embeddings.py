...
if __name__ == "__main__":
    # Generate embeddings
    ae_embeddings = generate_embeddings()
    
    # Print shape and sample embedding for verification
    print(f"\nEmbeddings shape: {ae_embeddings.shape}")
    print(f"Sample embedding (first row): {ae_embeddings[0]}")
    print(f"Sample embedding shape: {ae_embeddings[0].shape}")
    
    # Save embeddings to file
    output_path = 'embeddings.npy'
    print(f"\nSaving embeddings to {output_path}...")
    np.save(output_path, ae_embeddings)
